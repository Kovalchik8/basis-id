class Country {
  constructor() {
    this.country_select = $('form .country')
    this.input_phone = $('form input[name="phone"]')
    this.visitor_country = this.country_select.attr('data-country')
    this.countries = [
      {
        name: 'Afghanistan',
        flag:
  ...
  events() {
    $(document).ready(() => {
      this.init_country_select()
    })
    this.country_select.on('click', () =>
      this.country_select.find('ul').toggleClass('visible')
    )
  }
  ...
  init_country_select() {
    ...
    if (this.visitor_country.length)
      for (var i = 0; i < this.countries.length; i++) {
        if (this.countries[i].name == this.visitor_country) {
          ...
          <img src="${this.countries[i].flag}"> <span>${this.countries[i].name}</span>
          `)
          this.country_select.attr('data-code', this.countries[i].number)
          this.input_phone.val(this.countries[i].number)
          break
        }
      }

    // dropdown list for countries
    this.country_select.find('ul').append(`
        ${this.countries
          .map(
            item =>
    ...
    this.country_select
      .find('li')
      .on('click', this.country_item_onClick.bind(this))
  }

  country_item_onClick(e) {
    var target = $(e.target).is('li') ? $(e.target) : $(e.target).closest('li')
    this.country_select.find('.country__selected').html(target.html())
    this.country_select.attr('data-code', target.attr('data-code'))
    this.input_phone.val(target.attr('data-code'))
  }
}
